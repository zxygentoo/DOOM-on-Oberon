...
## -hw (feat/halftone: the hardware scanout ditherer)

`-hw` runs the shipped config with the video DMA live and the Halftone
...
FSM's `ht_ack`/`ht_row`/`ht_col`/`ht_word` probes — same file format,
