(* split_wad — the host-side WAD chunk splitter (AGENT.md §3; 2c's tooling half).
   ...
   Usage:  split_wad <file> [chunk_size_bytes]
...
let () =
  let input, chunk_size =
    ...
    | [ _; input; size ] ->
      (match int_of_string_opt size with
       | Some n -> input, n
       | None -> fail "split_wad: chunk size must be an integer: %s" size)
    | _ -> fail "usage: split_wad <file> [chunk_size_bytes]"
  in
  if chunk_size > Wad.po_file_cap
  then
    fail "split_wad: chunk size %d exceeds the PO file cap %d" chunk_size Wad.po_file_cap;
  let data = read_file input in
  if String.length data = 0 then fail "split_wad: %s is empty" input;
  ...
  List.iter
    (fun name ->
       ...
       then
         fail
           "split_wad: %S is %d chars; PO filenames cap at %d"
           oberon_name
  ...
  then fail "split_wad: chunk verification FAILED — reassembly differs from %s" input;
  List.iter2
    (fun name c ->
       Printf.printf
    ...
    chunks;
  Printf.printf
    "split_wad: %s -> %d chunks, %d bytes, reassembly verified\n"
    input
    (List.length chunks)
    (String.length data)
;;
