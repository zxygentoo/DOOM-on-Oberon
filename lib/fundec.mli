(** Per-function compilation (AGENT.md 1b): translate one integer *leaf* function —
    straight-line code, if/while/for control flow, and word-sized memory through the
    s3.2 address calculus (globals/statics off DB, array indexing, struct fields,
    deref chains, [&global], array decay, pointer arithmetic; offsets resolve through
    a {!Globals.t}) — from CIL's typed AST to a {!Emu.Risc5_isa.instr} list. Naive register allocation
