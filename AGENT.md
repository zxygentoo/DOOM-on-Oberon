...
1. **1-bit mono graphics, dithered.** No new video hardware — the existing
   ...
   ports). DOOM's smooth light gradients dither well under ordered dithering —
   v1 ships a 64×64 void-and-cluster **blue-noise** map at σ 1.5
   (`bin/bluenoise.ml` generates it; σ×γ judged by eye on contact sheets
   2026-07-09; Bayer 4×4 was the bring-up baseline) with default gamma 2
   (`patches/0002` — dithered 1-bit runs dark otherwise).
...
Blit (v1 — **fullscreen 1024×768 since 2026-07-09**, post-playtest; was
640×400 centered 2×2): 3.2×/3.84× stretch = DOOM's authentic 4:3 CRT pixel
aspect (3.84/3.2 = 1.2, exactly mode 13h — the square-pixel 2×2 render was
the squished rendition). Still ONE dither decision per source px: 10 source
px fill one fb word (3/3/3/3/4-bit replication, LSB leftmost), 25 source
lines fill 96 output lines (Bresenham). Compiled kernel ≈ 2 M instrs/frame
(frame 2.43 → 4.2 M avg) — the 1a hand-roll of `__dg_dither_fs` is the
queued ~2× lever, after the legibility pass settles kernel semantics.
Framebuffer stores ride write-through + wbuf; video reads come from the 10c
BRAM shadow — the video tax stays dead.
...
| **2c** ✅ | stub loader **complete (2026-07-09)**: `stub/DOOM.Mod` (~230 lines PO2013 Oberon-07, compiled in-image by the norebo ORP) — doomrun's loader obligations transliterated: blob → BLOB_BASE with the full §7 header verify (magic/version/length/checksum/entries, bss-range guard), `doom1.wad.N` chunks concatenated at WAD_BASE, SHARED zeroed + WAD length (+28) + command tail from `Oberon.Par` (+1024, rest-of-line so a bare Tool click doesn't swallow the lines below), bss zero, entries as procedure variables planted via `SYSTEM.PUT(SYSTEM.ADR(pv), addr)` — ORG proc-var calls put args in R0.. and the target in R[n] (`ORG.Mod` PrepCall/Call), the ABI §3 coincidence cashed. Seize loop = §6 path 2 made real: raw PS/2 off the MMIO FIFO (msAdr bit 28 / kbdAdr; F0/E0 state machine keeps the make/break + E0-arrows Input.Mod drops, swallows the fake-shift wrapper via `tabE[12H]=0`); letters stay ASCII (WASD rejected: every useful letter collides with cheat entry — iddqd needs its d). Quit → `Viewers.Broadcast` restore + status to the Log. `stub/mkdisk.sh` builds the bootable image: stock PO2013 (extracted from the pinned 2020-08-18 dsk) + the AgentTool serial channel (autoload patch) + DOOM.Mod + blob/chunks via `.packonly` → `DOOM.dsk`; tool repos vendored as pinned submodules (`vendor/oberon-risc-emu-rs` 7363fac — host tools only, its emulator can't model himem; `vendor/oberon-agent` 5a1481c) | **E1M1 plays on the booted OS** (vendored OCaml emulator = the 16 MiB himem machine; human-confirmed). Headless over the agent wire: full D_DoomMain banner on the UART, `oat call DOOM.Run -- "-timedemo demo1"` → **timed exactly 5026 gametics** — the desync-oracle count's third independent leg (host gcc ≡ doomrun ≡ the stub on the real OS) at 9.2 fps on the emulator's 25 MIPS clock — then the I_Error unwind → restore broadcast → `Oberon.Loop` resumed, proven end-to-end by the revived serial task flushing the Log delta (`DOOM: exit status -1` printed *after* Restore in the code). Rebuild from vendored submodules is byte-identical (md5 `a1e9db24…`) |
...
| **1d** ◐ | first frame of E1M1 **in simulation** — harness preloads blob+WAD straight into the PSRAM model; pixel-exact framebuffer dumps via the visual-golden harness, a bring-up luxury no DOOM port ever had. (0.39 M cyc/s ≈ 10 s/frame is *steady-state*; `D_DoomMain` init is hundreds of M cycles ≈ tens of sim-minutes — don't debug a "hang" that is `R_InitTextures`.) Then v1 stub on hardware. ◐ **the emulator leg RENDERS (2026-07-08)** — `bin/doomrun.ml`: plays the stub (blob at `BLOB_BASE`, WAD at `0xA00000`, SHARED page, bss zero per header), UART console on stdout, deterministic synthetic ms clock (default 44 000 steps/ms ≈ 60 MHz/CPI 1.37), Init/Tick through the real crt0 thunks, PGM frame dumps + FNV hashes, self-loop-trap detection. **Full `D_DoomMain` init completes (63.6 M instrs, Init → 0); TITLEPIC renders dithered into the exact §5 centered 640×400 rect; 400 Ticks clean; the E1M1 attract demo plays and renders in-game 3D frames.** First contact found 3 real bugs, each now a permanent gate: (1) the §8 WAD window was 1 716 B too small for the real shareware WAD — window end `0xE00000 → 0xE01000` (edited in place: the ABI has no consumers outside this repo yet); (2) **a live miscompile**: `materialize_addr`'s big-offset arm freed the base register before the two-instruction `load_const`, which landed ON it and doubled the offset — `&global[var]` as a value past the 16-bit immediate horizon (fopen's `&__handles[h]`; plain accesses ride the 20-bit mem-op offset and never see it) — jig sample `biga` fails-before/passes-after; (3) vsnprintf lacked *integer* precision (`%.3d` → `STCFN33`, HU_Init's font lump miss) — `vsn5` diffs it against genuine glibc. **The cfg mechanism (2026-07-09)**: §8's "cfg in" cashed as the SHARED `+1024` **command tail** — the loader writes raw text verbatim before Init (the Oberon stub will byte-copy `Oberon.Par` up to `~`, doomrun copies `-args`), Init prepends the baked `doom -iwad doom1.wad` and tokenizes in place (`__parse_cmdline`, jig `pcl1` direct + `pcl2` through the real Init to a recording fake); under `-timedemo` the engine is singletics, so heartbeat ≡ gametic and `-dump-every` dumps at exact gametics. **The desync oracle's count leg is GREEN**: emulator `-timedemo demo1` finishes at **exactly 5026 gametics** = the host build's constant — two independent implementations agree to the tic across 143 s of gameplay — and banks the project's **first fps number: 8.0** (naive allocator, 44 000 instrs/ms clock model — optimistic, no cache; 27.5 G instrs ≈ 5.5 M per rendered gametic). jig 238 samples / 11 134 green | **sim framebuffer golden ≡ host-reference frame ✅ (2026-07-09, emulator leg)** — `host/doomgeneric_golden.c` (headless CMAP256 TU, `-m32 -funsigned-char`, the SHIPPED `dither.c`, machine geometry) vs doomrun `-dump-at`: raw fb windows at gametics 100/1000/2500/5000 **bit-identical, 98 304 B each** — every pixel decision across 5 000 gametics of demo1 exact; alignment needed no game-memory access (singletics ⇒ gametic = Tick count + 1 on both sides); desync: count leg ✅ (5026 ≡ host), per-gametic *state checksum* (a vendor patch, both sides) — **open**, largely subsumed by the pixel oracle; timedemo fps ✅ first number 8.0 (naive alloc, optimistic clock model — Cyclesim owns the honest one); Cyclesim leg + on-hardware E1M1 — **open**. **Dither quality pass (2026-07-09, A/B under the oracle)**: Bayer → 64×64 void-and-cluster **blue noise** (`bin/bluenoise.ml`, deterministic — fixed seed/tie-breaks; the table in `dither.c` is the tool's verbatim default output; +4 080 B data, per-pixel cost unchanged) and **default gamma 2** (`patches/0002`, DOOM's own gammatable — the biggest visible win; F11 stays honest if ever wired). **σ 1.5 / γ 2 chosen by eye** on σ×γ contact sheets from a throwaway Python lab (a pixel-exact replay of the pipeline over captured scene inputs, validated against a real oracle frame before use; discarded after the decision — regrow it from this recipe if another taste pass ever comes: capture `DG_ScreenBuffer` + raw PLAYPAL at chosen gametics, replay gammatable→LUT→map→2×2 double; mind that `gammatable[0]` is NOT identity, so capture the palette pre-gamma or read PLAYPAL from the WAD). The golden oracle held bit-identical through every change — the shared-`dither.c` property live; `kd1` rewritten to endpoint-only vectors, valid for any 1..254 threshold map. Remaining shelf-knob: per-output-column decisions (640×400-grain blue noise, 2× cost) — gated on the perf numbers. **The perf campaign (2026-07-09, profile-driven)**: doomrun `-profile` (the flat-profile instrument) found the frame in four functions (dither 32% · span 23% · memcpy 22% · column 11%); the levers landed in measured order — memcpy word fast path (−15%), then the 1a drawers stocked with all three hot loops (`lib/drawers.ml`: dither 26→10.9 instrs/px branchless via the SUB-carry trick; span 2.55×; column 1.97×; each diffed against gcc compiling the very C it replaces, all golden-certified) — **5.17M → 2.43M instrs/tick (−53%)**. A loop-promotion pass (rung 2a) was built, measured at −0.6% (id's 1993 code already hoists), and **reverted** — measurement giveth and measurement taketh away. **M3 on the shipped-config Cyclesim: 7.35 Mcyc/tick = 8.2 fps at 60 MHz** (gameplay CPI 3.0 — the §1 cache-line prophecy; that hardware lever is now the next multiplier), golden ≡ host at gametic 100 on the hardware model with the optimized blob. **Fullscreen render + the input/on-system harnesses (2026-07-09, post-playtest)**: `dither.c` gains `__dg_dither_fs` (§5's new fullscreen geometry; `kdf` pins the masks/Bresenham/flip from endpoint vectors, `kd2` rewritten as the fullscreen DrawFrame witness, fences outside the panel); goldens regenerated, **bit-identical ×4** through the geometry change (the shared-`dither.c` property again), 5026 gametics again; compiled-kernel cost noted in §5, hand-roll queued. Two harness instruments landed with it: doomrun **`-keys`** (scripted `tick:doomkey:pressed` through the real KeyIn — the blob's whole ring/responder/menu path headless: menu-quit proven from attract *and* in-game) and **`bin/dskrun.ml`** (boots the *dsk* in-process: deterministic timeline of raw PS/2 hex + middle-clicks + §8 SHARED probes + PGM dumps — DOOM.Mod's PollKeys and the full click→play→quit→restore loop proven on the booted OS, desktop-restore screenshot included). The harness closed the first playtest bug report as **not-a-bug with a real finding under it**: "quit doesn't work" = the y/n confirmation *prompt renders illegibly* in the 1-bit dither (drawn — frame-diff bbox proves it — but small HU-font red text dithers into the background: ITU-601 puts DOOM's UI red at lum ≈ 56); input was never broken — `y` quits, ring probes 2→4→6→8, status 1. **The legibility pass is the next taste slice** (luminance weights / per-output-pixel decisions, contact sheets per the σ×γ recipe), and it gates the `__dg_dither_fs` hand-roll (kernel semantics must settle first) |
